...
void	move_enemies(t_game *game)
{
	...
	while (enemies[i])
	{
		...
		if (enemies[i]->direction == DIRECTION_DOWN && can_move(game,
				enemies[i], 0, FRAME_RATE))
			walk(game, enemies[i], 0, FRAME_RATE);
		else if (enemies[i]->direction == DIRECTION_UP && can_move(game,
				enemies[i], 0, -FRAME_RATE))
			walk(game, enemies[i], 0, -FRAME_RATE);
		else if (enemies[i]->direction == DIRECTION_RIGHT && can_move(game,
				enemies[i], FRAME_RATE, 0))
			walk(game, enemies[i], FRAME_RATE, 0);
		else if (enemies[i]->direction == DIRECTION_LEFT && can_move(game,
				enemies[i], -FRAME_RATE, 0))
			walk(game, enemies[i], -FRAME_RATE, 0);
		move_to_next_cell(enemies[i]);
...
int	check_collision(t_game *game, t_walker *player, t_walker *enemy)
{
	...
	if (abs(enemy_x - player_x) < IMG_SIZE && abs(enemy_y
			- player_y) < IMG_SIZE)
		quit(game, 0, "You've been elimenated by the monster :(");
	return (1);
}
...
int	enemy_can_move(t_game *game, t_walker *obj, int x, int y)
{
	...
	if (next_y < 0 || next_y > game->map_h - 1 || next_x < 0
		|| next_x > game->map_w - 1)
		return (0);
